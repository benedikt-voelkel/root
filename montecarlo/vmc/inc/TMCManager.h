// @(#)root/vmc:$Id$
// Author: Benedikt Volkel, 18/01/2019
...
// Class TMCManager
// ---------------------------
// Manager class for managing multiple TVirtualMC engines.
//
...
class TMCManager {
  ...
  public:
    ...
    void ForwardTrack(Int_t toBeDone, Int_t trackId, Int_t parentId,
    ...
    /// Cache the current geometry state and obtain its ID
    void CacheGeometryState(Int_t& cacheId);

    /// Transfer a track from engine with engineOriginId to engineTargetId
    void TransferTrack(Int_t engineOriginId, Int_t engineTargetId);
    ...
    void Apply(std::function<void(TVirtualMC*)> engineLambda);

    /// Initialize the engines
    void Init(std::function<void(TVirtualMC*)> initFunction);
