// @(#)root/vmc:$Id$
// Authors: Benedikt Volkel 07/03/2018
...
public:
   ...
   /// Flag a particle status as outdated (this is the case as soon as the
   ///                                     transport has started)
   // void ParticleStatusOutdated(Int_t trackId);
...
private:
   ...
   void ConnectTrackContainers(std::vector<TParticle *> *particles, std::vector<TMCParticleStatus *> *tracksStatus,
...
private:
   ...
   /// number of all primaries ever pushed
   Int_t *fTotalNPrimaries;
   /// number of all tracks ever pushed
   Int_t *fTotalNTracks;
   /// Tracks transient
   std::vector<TParticle *> *fParticles;
   /// All tracks (persistent except for the case the TMCManager requests
   ///             changes)
